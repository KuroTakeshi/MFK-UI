--[[
	...
	iRay  | Programming
	Max   | Programming
	Lorenzo | Re-Code & Re-Name XD

]]

if debugX then
	warn('Initialising MFK')
end
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_MFK_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local MFKFolder = "MFK"
local ConfigurationFolder = MFKFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		MFKOpen = {Type = 'bind', Value = 'K', Name = 'MFK Keybind'},
		-- buildwarnings
		-- MFKprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(MFKFolder) then
				if isfile and isfile(MFKFolder..'/settings'..ConfigurationExtension) then
					file = readfile(MFKFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"MFKOpen":{"Value":"K","Type":"bind","Name":"MFK Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"MFK Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('MFK had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("MFK", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify MFK developers at sirius.menu/discord.")
				end
			end)
...
local MFK = useStudio and script.Parent:FindFirstChild('MFK') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if MFK:FindFirstChild('Build') and MFK.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('MFK | Build Mismatch')
		print('MFK may encounter issues as you are running an incompatible interface version ('.. ((MFK:FindFirstChild('Build') and MFK.Build.Value) or 'No Build') ..').\n\nThis version of MFK is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, MFK = MFK, useStudio and script.Parent:FindFirstChild('MFK') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	MFK.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == MFK.Name and Interface ~= MFK then
			Interface.Enabled = false
			Interface.Name = "MFK-Old"
		end
	end
elseif not useStudio then
...
if MFK.AbsoluteSize.X < minSize.X and MFK.AbsoluteSize.Y < minSize.Y then
...
local Main = MFK.Main
local MPrompt = MFK:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = MFK:FindFirstChild('Drag')
...
MFK.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/MFK/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = MFK.Notifications

local SelectedTheme = MFKLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = MFKLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	MFK.Main.BackgroundColor3 = SelectedTheme.Background
	MFK.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	MFK.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	MFK.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	MFK.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	MFK.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	MFK.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		MFK.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		MFK.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(MFK:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("MFK | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("MFK | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('MFK had an issue decoding the configuration file, please try delete the file and reopen MFK.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(MFKLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("MFK | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--MFKLibrary:Notify({Title = "MFK Flags", Content = "MFK was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(MFKLibrary.Flags) do
...
function MFKLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('MFK | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			MFKLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show MFK'.", Duration = 7, Image = 4400697855})
		else
			MFKLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.MFKOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(MFKFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('MFK Settings', 0, true)

	if TabList['MFK Settings'] then
		TabList['MFK Settings'].LayoutOrder = 1000
	end

	if Elements['MFK Settings'] then
		Elements['MFK Settings'].LayoutOrder = 1000
	end
...
function MFKLibrary:CreateWindow(Settings)
	if MFK:FindFirstChild('Loading') then
		if getgenv and not getgenv().MFKCached then
			MFK.Enabled = true
			MFK.Loading.Visible = true

			task.wait(1.4)
			MFK.Loading.Visible = false
		end
	end

	if getgenv then getgenv().MFKCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				MFKLibrary:Notify({Title = 'Build Mismatch', Content = 'MFK may encounter issues as you are running an incompatible interface version ('.. ((MFK:FindFirstChild('Build') and MFK.Build.Value) or 'No Build') ..').\n\nThis version of MFK is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(MFKFolder) then
		makefolder(MFKFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "MFK"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "MFK Interface Suite" then
		LoadingFrame.Version.Text = "MFK UI"
	end
	...
	if not Settings.DisableMFKPrompts then
		task.spawn(function()
			while true do
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(MFKFolder.."/Discord Invites") then
			makefolder(MFKFolder.."/Discord Invites")
		end

		if isfile and not isfile(MFKFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(MFKFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"MFK RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(MFKFolder.."/Key System") then
			makefolder(MFKFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("MFK | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with MFK specific development.')
				end
			end
		end
		...
		if isfile and isfile(MFKFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(MFKFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(MFKFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						MFKLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("MFK | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with MFK specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= MFKLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					MFKLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			MFK.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("MFK | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with MFK specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					MFKLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("MFK | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with MFK specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					MFKLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("MFK | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with MFK specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					MFKLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= MFKLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("MFK | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with MFK specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						MFKLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("MFK | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with MFK specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					MFKLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			MFKLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			MFKLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('MFK had an issue creating settings.') end
...
function MFKLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function MFKLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function MFKLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['MFK Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.MFKOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function MFKLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				MFKLibrary:Notify({Title = "MFK Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			MFKLibrary:Notify({Title = "MFK Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('MFK Configurations Error | '..tostring(result))
			MFKLibrary:Notify({Title = "MFK Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = MFKLibrary:CreateWindow({
	...
	--MFKLibrary:Notify({Title = "MFK Interface", Content = "Welcome to MFK. These - are the brand new notification design for MFK, with custom sizing and MFK calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(MFKLibrary.Theme) do
